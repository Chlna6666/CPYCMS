  ...
  <div class="w-full pb-8">
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      >
        <div class="mb-3 flex flex-wrap items-start justify-between gap-3">
          <div class="flex flex-col gap-1.5">
            <div class="flex items-center gap-2">
              ...
              <span class="font-bold text-slate-900 dark:text-slate-100">{{ comment.nickname }}</span>
              <span
              ...
              <span class="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                ...
                {{ comment.email || t('adminStatus.noEmail') || '未留邮箱' }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <span
              v-if="comment.is_replied"
              class="inline-flex items-center gap-1 rounded-md bg-brand-50 px-2 py-1 text-xs font-medium text-brand-600 dark:bg-brand-900/20 dark:text-brand-400"
            >
        ...
        <div v-if="comment.article_slug" class="mb-3">
          <router-link
            :to="`/article/${comment.article_slug}`"
            class="inline-flex items-center gap-1.5 rounded-lg bg-cyan-50 px-2.5 py-1 text-xs font-medium text-cyan-600 transition-colors hover:bg-cyan-100 dark:bg-cyan-900/20 dark:text-cyan-400 dark:hover:bg-cyan-900/40"
            target="_blank"
          >
            <LinkIcon class="h-3.5 w-3.5" />
            {{ t('adminComment.articlePrefix') || '评论于文章：' }}{{ comment.article_title || comment.article_slug }}
          </router-link>
        </div>

        <p class="mb-4 whitespace-pre-wrap text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          {{ comment.content }}
        </p>
        ...
        >
          <div class="mb-1 flex items-center gap-1.5 text-xs font-bold text-brand-600 dark:text-brand-400">
            <MessageSquare class="h-3.5 w-3.5" />
            {{ t('adminComment.adminReplyPrefix') || '回复：' }}
          </div>
        ...
        <div class="grid gap-3 rounded-xl bg-slate-50 p-4 text-xs text-slate-600 dark:bg-slate-900/50 dark:text-slate-400 md:grid-cols-2">
          <div class="flex items-center gap-2">
            <Monitor class="h-4 w-4 shrink-0 text-slate-400" />
            <span class="truncate" :title="`${comment.browser} / ${comment.os_name} / ${comment.device_type}`">
              {{ t('adminComment.client') || '设备: ' }}{{ comment.browser }} / {{ comment.os_name }} / {{ comment.device_type }}
            </span>
          </div>
          <div class="flex items-center gap-2">
            <Globe class="h-4 w-4 shrink-0 text-slate-400" />
            <span class="truncate" :title="comment.ip_address">
              {{ t('adminComment.ip') || 'IP: ' }}{{ comment.ip_address || '-' }}
            </span>
          </div>
          <div class="flex items-center gap-2">
            <Fingerprint class="h-4 w-4 shrink-0 text-slate-400" />
            <span class="truncate" :title="comment.fingerprint_hash">
              {{ t('adminComment.fingerprint') || '指纹: ' }}{{ comment.fingerprint_hash || '-' }}
            </span>
          </div>
          <div class="flex items-center gap-2">
            <Info class="h-4 w-4 shrink-0 text-slate-400" />
            <span class="truncate" :title="comment.user_agent">
              {{ t('adminComment.userAgent') || 'UA: ' }}{{ comment.user_agent || '-' }}
            </span>
          </div>
        </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {{ comment.is_replied ? t('adminAction.editReply') || '修改回复' : t('adminAction.reply') || '回复' }}
          </button>
          ...
          >
            ...
            {{ t('adminAction.delete') || '删除评论' }}
          </button>
        </div>
      </article>

      <div v-if="comments.length === 0" class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white py-24 dark:border-slate-800 dark:bg-slate-950">
        <MessageSquareOff class="mb-4 h-12 w-12 text-slate-300 dark:text-slate-600" />
        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ t('adminEmpty.comments') || '暂无符合条件的评论' }}</p>
      </div>
    </div>
      ...
      <div v-if="showReply" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative flex w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl dark:bg-slate-950">
          <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
            <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">{{ t('adminComment.replyTitle') || '回复评论' }}</h3>
          ...
          <div class="px-6 py-5">
            ...
            <textarea
              ...
              rows="5"
              :placeholder="t('adminComment.replyPlaceholder') || '请输入回复内容...'"
            ></textarea>
          </div>
...
// 引入高颜值 Lucide 图标
import {
  User, Mail, Clock, ShieldAlert, CheckCircle, Link as LinkIcon,
  Monitor, Globe, Fingerprint, Info, Check, Trash2, Reply,
  MessageSquare, MessageSquareOff, ChevronLeft, ChevronRight, X, Send
} from '@lucide/vue'
...
async function loadData() {
  ...
  try {
    const res = await commentApi.adminGetList(params)
...
async function deleteComment(id: number) {
  if (!confirm(t('adminComment.deleteConfirm') || '确定要删除这条评论吗？此操作不可恢复。')) return
  try {
